...
const VISIBLE_DIST_THRESHOLD=0.4;
const VISIBLE_ANGLE_THRESHOLD=0.85;
...
func _check_and_make_visible(controller):
	if (active_controller): 
		global_transform = active_controller.get_palm_transform();
		_update_label_text();
		controller = active_controller;
	var trafo : Transform = controller.get_palm_transform();
